...
function startNewSession(req, res, user, connection) {
    ...
    // 기존 세션이 있으면 해당 세션을 메인화면으로 이동시키는 이벤트 발생
            ...
            if (password === user.PASSWORD) {
                ...
                if (user.session_id && user.session_id.trim() !== '') {
                    ...
                    req.session.user = {
                        id: user.ID,
                        name: user.NAME,
                        car: user.CAR,  // CAR 정보를 세션에 저장
                        role: user.ROLE
                    };

                    console.log('세션에 저장된 사용자 정보:', req.session.user); // 디버깅을 위한 로그
...
app.get('/driver3', (req, res) => {
    ...
    connection.query('SELECT CAR, SASI FROM bon_user WHERE ID = ?', [userId], (err, userResults) => {
    ...
    function processTotalValues(SASI, currentLocDigits, user) {
        connection.query('SELECT * FROM bon_planing_sin WHERE RESERVE IS NULL', (err, planResults) => {
            ...
            planResults.forEach(row => {
                ...
                // TOTAL의 오른쪽 세 자리 숫자가 12에서 110 사이인지 확인하고, TOTAL 값이 400,000 이하인지 확인
                if (totalRightThreeNum >= 12 && totalRightThreeNum <= 110 && totalValue <= 400000) {
                    console.log(`Checking row: ${row.ID}, TOTAL: ${row.TOTAL}, totalRightThree: ${totalRightThree}`);

                    // SANG_HA에서 접두사 추출 (예: HPNTC)
                    const sangHaPrefix = row.SANG_HA.split('-')[0];

                    // 접두사가 존재하고 CURRENT_LOCATION과 일치하는지 확인
                    if (sangHaPrefix && currentLocDigits !== sangHaPrefix) {
                        console.log(`No match for SANG_HA prefix ${sangHaPrefix} in CURRENT_LOCATION ${currentLocDigits}`);
                        return; // 일치하지 않는 경우, 다음 로직으로 넘어감
                    }

                    console.log(`Matching SANG_HA prefix with CURRENT_LOCATION: ${currentLocDigits} equals ${sangHaPrefix}`);
                    
                    // SASI에 따른 TOTAL 값 소수점 조건 확인
            ...
            if (selectedRow) {
                ...
                console.log('적합한 TOTAL 값을 찾지 못했습니다.');
    ...
    function assignTotalValue(row, user) {
        ...
        connection.query('UPDATE bon_planing_sin SET RESERVE = "Y" WHERE TOTAL = ?', [row.TOTAL], (err) => {
            ...
            console.log(`Assigned TOTAL value ${row.TOTAL}.`);
    
            connection.query('UPDATE bon_carplayer SET CURRENT_LOCATION = ? WHERE CAR = ?', [selectedSangHa, user.CAR], (err) => {
                if (err) {
                    console.error('bon_carplayer 업데이트 중 오류:', err);
                    connection.end();
                    return res.status(500).send('내부 서버 오류');
                }
    
                console.log(`Updated CURRENT_LOCATION to ${selectedSangHa} for CAR ${user.CAR}.`);
                
                // 배차 데이터와 SASI 값을 세션에 저장
